...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086542841-cb30d513-7976-4449-9c2d-9b93fc801cc0.png)
...
### 什么是 Agent Loop？
Agent Loop = Agent不断思考、行动、观察、再思考的循环过程。  

它每一轮大概做三件事：让 LLM 推理，调用工具，把工具结果写回上下文。一直循环，直到任务完成或者触发停止条件。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086542849-da84c3b0-11c7-49b7-97a8-268923bfbb3c.png)
...
纯代码封装、调用路径固定，用 Toolkits。团队经验沉淀、任务流程灵活，用 Agent Skills 更合适。更详细的 Skills 工程实践——包括路由设计、[SKILL.md](http://skill.md/) 写法避坑、第三方 Skill 安全审计，可以看：《Agent Skills 详解》。

### 通信接入：MCP 协议
Function Calling 解决“模型怎么调用工具”，MCP 解决“工具怎么接入系统”。

前者是 AI 的决策协议，后者是 AI 工具的接入协议。Function Calling 让模型会用工具，MCP 让开发者不用手写大量工具接线代码。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086542930-ff4bd2a2-aec5-46f9-a724-c3a7904434d9.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086543150-b494e4b7-c105-4f9c-92ba-bd3a78528725.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086543100-ee2cc2a1-c33f-4adf-b027-0eff43d626e4.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086543429-00d2ecfa-57b6-4e67-983a-5461a0df7780.png)
...
### Plan-and-Execute
Plan-and-Execute（规划-执行）是 Agent 领域非常经典的一种架构，可以理解为：

> **先让大模型制定计划（Plan），再按照计划逐步执行（Execute）。**
>

它是对 ReAct Agent 的一种优化。



Plan-and-Execute 是一种 Agent 架构模式，其核心思想是将  **决策（Planning）**和** 执行（Execution）**分离。

在这种架构中，大模型首先根据目标生成一份任务计划，然后执行模块按照计划完成各项任务。规划阶段负责回答“要做什么”，执行阶段负责回答“怎么做”。

从架构角度看，Plan-and-Execute 包含三个核心组件：
...
即在执行过程中持续评估结果，并根据反馈重新生成计划。

从本质上说：

**Plan-and-Execute 是一种“先全局规划，再局部执行”的 Agent 设计思想，通过把任务规划权和任务执行权分离，提高复杂任务的完成质量和可控性。**
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086543290-6f8f22b4-3d6c-4501-8b84-5480609679f0.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086543221-0613b652-af03-4e04-a7bc-b955c52166db.png)

### Agentic Workflows
他的观点很务实：没必要一直干等底层模型突破。用工程方法，把推理、工具、记忆、反思、多实体协作编排成流水线，已经能做出很多可用的 AI 应用。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1781086543546-532d6478-c5ac-4e0f-9584-c90ff7195f2d.png)
...
这套图结构比写死的 if-else 链更容易扩展，出了问题也好定位到哪个节点哪条边。LangGraph（Python）和 Spring AI Alibaba Graph（Java）都是基于这套思路实现的。

...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1780984646938-caa4fcb3-c811-48b0-acd7-40266b6d9ba8.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1780984646809-dea8fa9a-cd67-40c6-a848-4c5372833749.png)
...
+ 只做**轻量数据抽取**，可以先用 Structured Outputs。
+ 需要**读写业务系统**，优先考虑 Function Calling / Tool Calling。
...
自由文本的问题是不可控：后端到底把 `urgent` 当成 `HIGH`，还是当成非法值？如果你在服务端做模糊映射，就相当于把模型的不确定性扩散到了业务规则里。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/58546395/1780984646824-4eaae252-a786-42c5-9de9-125caad45083.png)
...
MCP Tool 是对 HTTP API 的**标准化封装**，让 LLM 能以统一方式发现和调用各种外部服务，而不需要为每个 API 单独写适配代码。MCP 本质是给 LLM 定义了一套"工具调用标准"，HTTP API 是它底层的实现细节，两者是封装与被封装的关系。  

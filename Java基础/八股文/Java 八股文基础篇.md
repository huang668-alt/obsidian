...
- **对象实体（Object Entity）**： 也称为“对象本身”或“对象实例”，是指在**堆内存（Heap）**中实际分配出来的那块内存空间，里面存放了对象的属性值（实例变量）和方法表指针等真实数据。
- **对象引用（Object Reference）**： 是一个**变量**，它的值是一个指向堆中对象实体的“地址”或“指针”。它本身存放在**栈内存（Stack）**（如果是局部变量）或堆中（如果是实例变量或静态变量）。引用本身不存放对象的数据，只存放对象的“位置”。
...
- 对于**哈希值相同**的节点，则会进一步调用 equals() 方法来检查这两个对象是否“相等”：  
    – 如果 `equals()` 返回 true，说明集合中已经存在与当前对象等价的元素，`HashSet` 就不会再次加入它；– 如果返回 false， 则认为是新元素，会将该对象作为一个新节点加入到**同一个桶**的链表或红黑树中。

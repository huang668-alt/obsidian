...
###### 1. 底层黑魔法：隐式的引用和合成方法（Synthetic Method）
...
### 一. 字符串艺术（String、StringBuilder、StringBuffer）

#### 一、 String：不可变的“内存钉子户”

一句话点名本质：**`String` 对象一旦在堆内存中诞生，它的内容就绝对无法被修改。**
##### 1. 底层源码的锁死机制

如果我们翻看 `String` 的官方源码（Java 9 之后），会看到它是这样定义的：

``` Java
public final class String implements java.io.Serializable, Comparable<String>, CharSequence {
    @Stable
    private final byte[] value; // 底层实打实的存储数组
}
```

- **`final class`**：锁死了类，代表 `String` 不能有子类，防止坏人通过继承去篡改核心逻辑。
    
- **`final byte[] value`**：锁死了引用的物理数组。一旦这个数组被初始化分配了长度和内容，它在内存中的二进制数据就**再也无法被二次写入**。
...
在 JVM 底层，**绝对没有**在原先的数组后面追加 `" World"`。相反，内存中发生了一场大搬家：

1. JVM 在堆里开辟了一块新空间，`new` 了一个全新的 `String` 对象。
    
2. 新对象内部生成了一个容量更大的数组，把 `"Hello"` 和 `" World"` 复制进去。
    
3. 变量 `s` 掐断原先的连接，指向这个新对象的内存地址。
    
4. 原先的 `"Hello"` 对象彻底沦为没人要的**内存垃圾**，等待 GC（垃圾回收器）来收尸。

> **🎯 适用场景**：少量的字符串操作，或者定义字面量常量。由于它不可变，多线程同时读取它时**天然线程安全**。
#### ⚡ 二、 StringBuilder：追求极致速度的“单线程推土机”

为了解决 `String` 频繁拼接导致堆内存被垃圾撑爆的惨剧，Java 提供了 `StringBuilder`。

一句话点名本质：**它是一个长在堆内存里、支持原地动态扩容的“可变”字符序列。**
##### 1. 源码中的可变基因

`StringBuilder` 继承自 `AbstractStringBuilder`，我们去看它的底层：
``` Java
abstract class AbstractStringBuilder implements Appendable, CharSequence {
    byte[] value; // 注意：这里没有 final！它是普通的、可变的数组
    int count;    // 记录当前真正装了多少个字符
}
```

因为没有 `final` 强制锁死，当你调用 `.append(" World")` 时，JVM 是直接**在原数组的空余格子里写入新数据**，不会产生任何多余的垃圾对象！
##### 2. 底层的“自我进化”：动态扩容机制

如果原数组满了，装不下了怎么办？`StringBuilder` 会触发自动扩容：

1. **计算新容量**：底层通过位运算，将新容量扩大为 **$2 \times \text{原容量} + 2$**。
    
2. **硬件级拷贝**：在堆内存的其他地方开辟这块新大空间，利用底层的 `System.arraycopy()` 汇编级指令，将旧数据高带宽复制到新空间。
    
3. **指针替换**：让内部的 `value` 变量指向新数组，旧数组被回收。

##### 3. 致命缺陷：高并发下会“掉数”

`StringBuilder` 的所有方法都是纯裸奔的，**没有加任何并发锁**。

如果两个线程同时往一个 `StringBuilder` 里 `append` 数据，它们可能会同时拿到同一个 `count` 下标，导致线程 A 写入的数据直接被线程 B **覆盖掉（发生数据撕裂/掉数）**，甚至可能触发 `ArrayIndexOutOfBoundsException` 数组越界崩溃。

> **🎯 适用场景**：**单线程**环境下，需要进行大量的字符串拼接、删除、修改操作（如循环拼接 SQL、组装 JSON 串）。性能毁天灭地。

#### 🔒 三、 StringBuffer：全身重甲的“多线程防弹车”

`StringBuffer` 和 `StringBuilder` 是一对孪生兄弟，它们的底层动态扩容机制、数组结构**完全一模一样**。

它们唯一的区别，就在于**安全性**。

##### 1. 源码中的“无脑加锁”

如果我们去翻看 `StringBuffer` 的源码，会发现它的每一个核心方法（如 `append`, `insert`, `delete`）前面，都死死地扣着一个重型防御盾牌：
...
- **`synchronized`（同步锁）**：这意味着，当线程 A 在往这个大箱子里装字的时候，整个方法直接被**死死锁住**。其他线程必须在门外老老实实排队，等线程 A 彻底执行完放开锁后，下一个线程才能进来。

##### 2. 性能的惨烈代价

因为高频的加锁、解锁需要向操作系统申请复杂的互斥量（Mutex），这会带来极为沉重的 CPU 上下文切换开销。在现代高并发场景下，多个线程疯狂争抢同一把 `synchronized` 锁，会导致程序流速发生严重的**严重堵塞**。

> **🎯 适用场景**：极其罕见的、明确需要**多线程共享并同时修改同一个字符串容器**的古董级并发场景。在现代开发中，这个类基本已经退居二线。

### 二. Java 集合框架（Collections）

[[Java 集合基础知识]]

### 三. 异常处理（Exception）

在 Java 知识体系中，**异常处理（Exception Handing）**是一套独立于正常业务逻辑之外的**底层防御机制**。很多程序员只把 `try-catch` 当作不让程序崩溃的“创可贴”，但如果你去翻看 JVM 规范，你会发现异常在运行期的**抛出、栈轨迹追踪、以及控制流跳转**，在底层有着极其沉重的物理代价。
#### 一、 异常树门派体系：JVM 眼中的灾难等级

Java 中所有的异常和错误都派生自同一个老祖宗：`java.lang.Throwable`。在 JVM 眼里，灾难被严密地划分为两大阵营：
##### 1. Error（错误）：不可挽救的顶层崩溃

- **核心本质**：属于 JVM 级别的严重故障，或者硬件资源耗尽。
    
- **经典代表**：
    
    - `OutOfMemoryError`（OOM）：堆内存被对象塞满了，垃圾回收器竭尽全力也腾不出空间。
        
    - `StackOverflowError`：栈深度溢出。通常是因为没有边界的死循环递归，导致线程栈帧把 1MB 的栈内存撑爆了。
        
- **底层的态度**：**程序无法自救**。JVM 不期望你用 `try-catch` 去捕获它们，一旦发生，整个线程甚至整个 JVM 进程通常会直接暴毙。

##### 2. Exception（异常）：可以挽救的程序断点

程序运行中发生的非致命问题，可以通过编码手段进行修复或优雅兜底。它又分为两个派系：

- **编译时异常（Checked Exception / 受检异常）**：
    
    - **底层特质**：除了 `RuntimeException` 及其子类以外的异常（如 `IOException`, `SQLException`）。
        
    - **编译期铁律**：Java 编译器（javac）在编译时会死死盯着这类异常。如果你调用了一个声明抛出受检异常的方法，你必须显式地用 `try-catch` 捕获，或者在方法签名上 `throws` 抛出，否则**代码连编译都过不去，根本无法生成 .class 字节码**。
        
- **运行时异常（RuntimeException / 非受检异常）**：
    
    - **底层特质**：诸如 `NullPointerException`（空指针）、`ArrayIndexOutOfBoundsException`（数组越界）、`ClassCastException`（类型强转失败）。
        
    - **编译期放行**：这类异常通常是由于**代码逻辑不够严谨**导致的。编译器在编译时会直接放行，但如果在运行期真的触发了错误逻辑，JVM 会在运行时瞬间抛出。

#### 二、 运行期的底层黑魔法：异常表（Exception Table）

很多人以为，当程序运行到 `try` 块时，JVM 会在底层启动一个类似“监视器”的东西死死盯着这几行代码。

**完全错误！在 JVM 层面，`try-catch` 机制不会对正常执行的代码产生任何额外的监控开销。** 它的底层核心全部依赖于 `.class` 字节码文件里的一张秘密表格——**异常表（Exception Table）**。
##### 1. 异常表的字节码真面目

假设我们写下了这段极其普通的异常捕获代码：
...
我们使用 `javap -v` 反编译这个类，在方法字节码的最后，你会看到这样一张清晰的表格：

> **Exception table:**
> 
> |**from**|**to** |**target**|**type**|
> |------|---|-------|---|
> |       0|12|      1 5|Class java/lang/ArithmeticException|

- **`from` 和 `to`**：代表 `try` 块内代码对应的**字节码指令指令范围**（从第 0 行到第 12 行指令）。
    
- **`target`**：代表如果发生异常，跳转去的 **`catch` 块的指令起点**（第 15 行指令）。
    
- **`type`**：代表该监控范围对哪种**异常类型**生效。

##### 2. JVM 的连环寻址全流程

当代码执行到 `int i = 1 / 0;` 时，CPU 触发硬件除零错误，JVM 捕获该硬件信号并实例化一个 `ArithmeticException` 对象。接着，JVM 开始执行异常查找流程：

1. **查表定位**：JVM 会看一眼当前执行到了哪一行字节码指令。
    
2. **范围匹配**：拿着当前指令行号，去该方法的异常表（Exception Table）里自上而下逐行扫描，看这个行号落在哪个 `from-to` 范围内。
    
3. **类型匹配**：如果行号匹配成功，JVM 接着比对当前抛出的异常对象，是不是表中 `type` 列声明的类或其子类。
    
4. **精确跳转**：如果类型也对上了，JVM 会强行改变程序计数器（PC 寄存器）的值，将其直接跳转到 `target` 所指向的第 15 行字节码指令（即进入 catch 块）执行。
    
5. **弹栈崩溃**：如果遍历完异常表发现**没有任何一行能匹配上**，JVM 就会将当前方法的栈帧**直接弹出（销毁）**，把异常抛给调用它的上一级方法（父栈帧）。如果一路弹到 `main` 方法的主栈帧还没人接盘，整个线程直接宣告崩溃挂掉。

#### 三、 为什么说异常的代价极其昂贵？

很多高级架构师都会告诫新手：**绝对不要用 `try-catch` 来做业务逻辑的判断（比如用捕获空指针来代替 `if (obj != null)`）。** 这绝非强迫症，而是因为抛出异常时，JVM 在底层做了一件**极其消耗 CPU 资源和内存资源**的苦力活——**栈轨迹填充（Fill in Stack Trace）**。

##### 1. 致命的 `fillInStackTrace()`

当我们初始化任何一个异常对象（`new Exception()`）时，其父类 `Throwable` 的构造方法里会默认调用一个原生本地方法（Native Method）：

``` Java
public synchronized Throwable fillInStackTrace();
```

这个方法一旦触发，JVM 会瞬间暂停当前的正常执行流，开始痛苦的“剥洋葱”工程：

- 它会去内存里自底向上**遍历当前线程的整个调用栈**。
    
- 挨个把当前栈里的每一层栈帧（Frame）的信息全部读取出来，包括：类名、方法名、文件名、代码执行的精确行号。
    
- 把这些庞大的元数据信息打包、压缩，最终填充进异常对象的内部字段中。
    

**一句话点名代价**：如果你的项目框架很深（比如用上了 Spring Boot + Tomcat，一个业务调用的深度常常能达到几十甚至上百层栈帧），**每一次抛出异常，JVM 就要把这上百层楼爬一遍并拍照做记录。这个开销，比普通的 `if-else` 分支逻辑慢了至少数百倍！**
...
很多人都知道，不管 `try` 块里有没有 `return`，`finally` 块里的代码都一定会执行。JVM 在底层究竟是怎么做到这种绝对忠诚的？

##### 底层的真相：复制粘贴

在早期的 JVM 中，`finally` 依靠 `jsr` 汇编指令来实现跳转；但在现代 JVM 中，为了提高效率，编译器采用了更粗暴的做法：**“代码复制”**。

在编译成字节码时，编译器会把 `finally` 块内部的所有指令，**偷偷复制并粘贴到了整个方法的每一个出口处**！

- 粘贴在 `try` 块代码结束、准备 `return` 的前一步。
    
- 粘贴在 `catch` 块代码结束、准备 `return` 的前一步。
    
- 粘贴在所有异常表 `target` 跳转、准备把异常往外抛的前一步。

也就是说，在底层根本没有什么神奇的动态折返跳转，纯粹是**编译器把 `finally` 的代码在所有可能的自杀或退出通路上，都提前铺好了一张安全网**。

#### 五、 异常处理的高级编码避坑指南

1. **不要直接 `catch (Throwable)` 或 `catch (Exception)`**：
    
    这被称为“异常吞噬”。它会把底层的真正错误（甚至是 OOM 或核心中断）和普通业务异常一锅端地拦下来。一旦拦截后又没打好日志，整个系统就会陷入“明明报错了，但没有任何线索”的诡异黑洞。
    
2. **尽量用 `if` 代替异常判空**：
    
    能用防御性代码（如 `if (list == null || list.isEmpty())`）规避的运行时异常，绝对不要等到触发异常再去 catch，要全力避免 `fillInStackTrace()` 的触发。
    
3. **善用 Java 7 的 try-with-resources 语法糖**：
    
    对于需要关闭的流（如 `InputStream`、`BufferedReader`），改用 `try (Resource res = new Resource()) { ... }`。编译器会在底层自动帮你生成一整套严密的、包含防漏处理的 `finally { res.close(); }` 字节码，既优雅又绝对安全。
### 四. 高级黑魔法（泛型与反射）

这两者构成了现代流行开源框架（如 Spring 的 IOC/AOP、MyBatis、Android 的 Jetpack）的底层灵魂。

#### 1. 泛型（Generics）与“伪泛型”擦除机制
...
list.add("hello");
// 在编译后的字节码里，List<String> 变成了普通的 List，String 变成了 Object！
```

- **类型擦除（Type Erasure）**：Java 的泛型只存在于**编译期**，用来让编译器帮我们做类型安全检查。一旦编译通过，变成 `.class` 字节码后，所有的泛型信息都会被彻底抹去（擦除为第一个边界，通常是 `Object`）。
    
- **底层证据**：运行期你想通过 `list.add(123)` 会报错，但如果你利用**反射**绕过编译器，强行往这个 `List<String>` 里 `add` 一个整数 `123`，程序竟然可以完美运行！这就是擦除机制的铁证。

#### 2. 反射（Reflection）—— 动态性的究极奥义

反射赋予了程序“在运行期，通过一个类名字符串，直接把这个类全盘剥光并任意操控”的神性能力。
``` Java
// 运行期动态获取类，动态 new 对象，动态调私有方法
...
- **底层的代价**：反射虽然极其强悍，但在企业级开发中必须慎用，因为它的底层代价极其高昂：
    
    1. **性能损耗**：反射调用方法（`method.invoke()`）时，JVM 无法进行任何前面提到的 `final` 静态绑定和方法内联优化。它必须走一套繁琐的：安全检查 ➡️ 参数装箱拆箱 ➡️ 动态寻址 ➡️ 方法表扫描流程，性能慢。
        
    2. **安全破坏**：反射可以通过 `setAccessible(true)` 直接强行暴力震碎 `private` 访问权限修饰符，直接染指人家的私有核心属性，破坏了封装性。
...
###  一、 Java I/O 流与网络编程（操作系统文件与网卡交互）

这是程序与外部世界交换数据的核心通道。数据不管是存入固态硬盘（SSD），还是通过网线发给远程服务器，底层都是字节的流动。

#### 1. 传统的 BIO（Blocking I/O，阻塞型流）

- **物理过程**：当你用 `FileInputStream` 读取磁盘文件，或者用 `Socket` 读取网络数据时，如果对方的数据还没准备好，你的线程就会在操作系统内核触发 **`read()` 系统调用**，然后这个线程就会被**死死挂起（阻塞状态）**，什么也干不了，直到数据全部拷贝到 JVM 内存。
    
- **底层重叠**：传统的字节流（`InputStream/OutputStream`）和字符流（`Reader/Writer`）在底层都依赖操作系统的缓冲区。

#### 2. 高阶的 NIO（Non-blocking I/O，非阻塞与零拷贝）

- **物理本质（零拷贝 Zero-Copy）**：传统 I/O 需要把数据从磁盘 ➡️ 操作系统内核 ➡️ JVM 堆内存，来回拷贝好几次，CPU 开销极大。
    
- 现代 Java 的 NIO 引入了 **`Channel`（通道）** 和 **`Buffer`（直接内存缓存）**。它允许 Java 程序直接向操作系统申请操作系统的**物理内存（Direct Memory）**，跳过 JVM 堆，甚至利用操作系统底层的 `sendfile` 搞出“零拷贝”，让数据不经过 CPU 中转，直接在网卡和磁盘之间高速对冲！